...
$poizvedba = "SELECT datum_poteka FROM oglasi WHERE id = ? AND id_uporabnika = ? LIMIT 1";

$stmt = $conn->prepare($poizvedba);
$stmt->bind_param("ii", $_GET["oglas"], $_SESSION["login_id"]);
$stmt->execute();
$stmt->bind_result($datum_poteka);
$stmt->fetch();
...
if (date('Y-m-d') >= $datum_poteka) {
   if (isset($_POST["shrani1"]) && isset($_POST["naziv"])) {
      ...
      $poizvedba = "UPDATE oglasi SET naziv = ? WHERE id = ? AND id_uporabnika = ?";

      $stmt = $conn->prepare($poizvedba);
      $stmt->bind_param("sii", $_POST["naziv"], $_GET["oglas"], $_SESSION["login_id"]);
      $stmt->execute();
   ...
   if (isset($_POST["shrani2"]) && isset($_POST["opcija"])) {
      $poizvedba = "UPDATE oglasi SET opcija = ? WHERE id = ? AND id_uporabnika = ?";

      $stmt = $conn->prepare($poizvedba);
      $stmt->bind_param("sii", $_POST["opcija"], $_GET["oglas"], $_SESSION["login_id"]);
      $stmt->execute();
   ...
   if (isset($_POST["shrani3"]) && isset($_POST["kategorija"])) {
      $poizvedba = "UPDATE oglasi SET kategorija = ? WHERE id = ? AND id_uporabnika = ?";

      $stmt = $conn->prepare($poizvedba);
      $stmt->bind_param("sii", $_POST["kategorija"], $_GET["oglas"], $_SESSION["login_id"]);
      $stmt->execute();
   ...
   if (isset($_POST["shrani4"]) && isset($_POST["stanje"])) {
      $poizvedba = "UPDATE oglasi SET stanje = ? WHERE id = ? AND id_uporabnika = ?";

      $stmt = $conn->prepare($poizvedba);
      $stmt->bind_param("sii", $_POST["stanje"], $_GET["oglas"], $_SESSION["login_id"]);
      $stmt->execute();
   ...
   if (isset($_POST["shrani5"]) && isset($_POST["cena"])) {
      ...
      $poizvedba = "UPDATE oglasi SET cena = ? WHERE id = ? AND id_uporabnika = ?";

      $stmt = $conn->prepare($poizvedba);
      $stmt->bind_param("sii", $_POST["cena"], $_GET["oglas"], $_SESSION["login_id"]);
      $stmt->execute();
   ...
   if (isset($_POST["shrani6"]) && isset($_POST["opis"])) {
      ...
      $poizvedba = "UPDATE oglasi SET opis = ? WHERE id = ? AND id_uporabnika = ?";

      $stmt = $conn->prepare($poizvedba);
      $stmt->bind_param("sii", $_POST["opis"], $_GET["oglas"], $_SESSION["login_id"]);
      $stmt->execute();
   ...
   if (isset($_POST["shrani0"]) && isset($_FILES["slika"]) &&  !isset($_POST["odstrani"])) {
      $poizvedba = "SELECT s.slika1, s.slika2, s.slika3, s.slika4, s.slika5 FROM slike_oglasov s INNER JOIN oglasi o ON (s.id_oglasa=o.id) WHERE o.id = ? AND o.id_uporabnika = ? LIMIT 1";
      ...
      if (count($_FILES["slika"]["name"]) <= $pom) {
         ...
         if (in_array($_FILES["slika"]["type"][0], $dovoljeni_tipi) || in_array($_FILES["slika"]["type"][1], $dovoljeni_tipi) || in_array($_FILES["slika"]["type"][2], $dovoljeni_tipi) || in_array($_FILES["slika"]["type"][3], $dovoljeni_tipi) || in_array($_FILES["slika"]["type"][4], $dovoljeni_tipi)) {
            if (array_sum($_FILES["slika"]["size"]) < 4000000*$pom) {
               ...
               if ($pom == 5) {
                  $poizvedba = "UPDATE slike_oglasov SET slika1 = ?, slika2 = ?, slika3 = ?, slika4 = ?, slika5 = ? WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";

                  $stmt = $conn->prepare($poizvedba);
                  $stmt->bind_param("sssssii", $pom_slika1, $pom_slika2, $pom_slika3, $pom_slika4, $pom_slika5, $_GET["oglas"], $_SESSION["login_id"]);
                  $stmt->execute();

                  $stmt->close();
               }
               else if ($pom == 4) {
                  $poizvedba = "UPDATE slike_oglasov SET slika2 = ?, slika3 = ?, slika4 = ?, slika5 = ? WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";

                  $stmt = $conn->prepare($poizvedba);
                  $stmt->bind_param("ssssii", $pom_slika1, $pom_slika2, $pom_slika3, $pom_slika4, $_GET["oglas"], $_SESSION["login_id"]);
                  $stmt->execute();

                  $stmt->close();
               }
               else if ($pom == 3) {
                  $poizvedba = "UPDATE slike_oglasov SET slika3 = ?, slika4 = ?, slika5 = ? WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";

                  $stmt = $conn->prepare($poizvedba);
                  $stmt->bind_param("sssii", $pom_slika1, $pom_slika2, $pom_slika3, $_GET["oglas"], $_SESSION["login_id"]);
                  $stmt->execute();

                  $stmt->close();
               }
               else if ($pom == 2) {
                  $poizvedba = "UPDATE slike_oglasov SET slika4 = ?, slika5 = ? WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";

                  $stmt = $conn->prepare($poizvedba);
                  $stmt->bind_param("ssii", $pom_slika1, $pom_slika2, $_GET["oglas"], $_SESSION["login_id"]);
                  $stmt->execute();

                  $stmt->close();
               }
               else if ($pom == 1) {
                  $poizvedba = "UPDATE slike_oglasov SET slika5 = ? WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";

                  $stmt = $conn->prepare($poizvedba);
                  $stmt->bind_param("sii", $pom_slika1, $_GET["oglas"], $_SESSION["login_id"]);
                  $stmt->execute();
               ...
               $msg = "Skupna velikost objavljenih slik je prevelika! Maksimalna velikost je 20MB";
               $klasa = "alert-danger";
            }
         }
         else {
   ...
   if (isset($_POST["odstrani"]) && !isset($_POST["shrani0"]) && (isset($_POST["izbrisi1"]) || isset($_POST["izbrisi2"]) || isset($_POST["izbrisi3"]) || isset($_POST["izbrisi4"]) ||isset($_POST["izbrisi5"]))) {
      if (isset($_POST["izbrisi1"])) {
         $poizvedba = "UPDATE slike_oglasov SET slika1 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";
      ...
      if (isset($_POST["izbrisi2"])) {
         $poizvedba = "UPDATE slike_oglasov SET slika2 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";
      ...
      if (isset($_POST["izbrisi3"])) {
         $poizvedba = "UPDATE slike_oglasov SET slika3 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";
      ...
      if (isset($_POST["izbrisi4"])) {
         $poizvedba = "UPDATE slike_oglasov SET slika4 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";
      ...
      if (isset($_POST["izbrisi5"])) {
         $poizvedba = "UPDATE slike_oglasov SET slika5 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";
      ...
      if (empty($slika1) && !empty($slika2) && !empty($slika3) && !empty($slika4) && !empty($slika5)) {
         $poizvedba = "UPDATE slike_oglasov SET slika1 = ?, slika2 = ?, slika3 = ?, slika4 = ?, slika5 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";

         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("ssssii", $slika2, $slika3, $slika4, $slika5, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 00111
         ...
         $poizvedba = "UPDATE slike_oglasov SET slika1 = ?, slika2 = ?, slika3 = ?, slika4 = NULL, slika5 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";

         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("sssii", $slika3, $slika4, $slika5, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 00011
         ...
         $poizvedba = "UPDATE slike_oglasov SET slika1 = ?, slika2 = ?, slika3 = NULL, slika4 = NULL, slika5 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";

         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("ssii", $slika4, $slika5, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 00001
         ...
         $poizvedba = "UPDATE slike_oglasov SET slika1 = ?, slika2 = NULL, slika3 = NULL, slika4 = NULL, slika5 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";

         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("sii", $slika5, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 10111
         ...
         $poizvedba = "UPDATE slike_oglasov SET slika2 = ?, slika3 = ?, slika4 = ?, slika5 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";
      ...
      // 10001
         ...
         $poizvedba = "UPDATE slike_oglasov SET slika2 = ?, slika3 = NULL, slika4 = NULL, slika5 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";
      ...
      // 11011
         ...
         $poizvedba = "UPDATE slike_oglasov SET slika3 = ?, slika4 = ?, slika5 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";
      ...
      // 11001
         ...
         $poizvedba = "UPDATE slike_oglasov SET slika3 = ?, slika4 = NULL, slika5 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";
      ...
      // 11101
         ...
         $poizvedba = "UPDATE slike_oglasov SET slika4 = ?, slika5 = NULL WHERE id_oglasa IN (SELECT id FROM oglasi WHERE id = ? AND id_uporabnika = ?)";
      ...
      // 01110
         ...
         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("sssii", $slika2, $slika3, $slika4, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 01100
         ...
         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("ssii", $slika2, $slika3, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 01000
         ...
         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("sii", $slika2, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 00110
         ...
         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("ssii", $slika3, $slika4, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 00010
         ...
         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("sii", $slika4, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 01010
         ...
         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("ssii", $slika2, $slika4, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 10101
         ...
         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("ssii", $slika3, $slika5, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 00100
         ...
         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("sii", $slika3, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 01001
         ...
         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("ssii", $slika2, $slika5, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 01011
         ...
         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("sssii", $slika2, $slika4, $slika5, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
      ...
      // 01101
         ...
         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("sssii", $slika2, $slika3, $slika5, $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
...
else if (date('Y-m-d') < $datum_poteka && (isset($_POST["shrani1"]) || isset($_POST["shrani2"]) || isset($_POST["shrani3"]) || isset($_POST["shrani4"]) || isset($_POST["shrani5"]) || isset($_POST["shrani6"]) || isset($_POST["shrani0"]) || isset($_POST["odstrani"]) || isset($_POST["ponovno_objavi_oglas"]) || isset($_POST["izbrisi_oglas"]) || isset($_POST["odstrani_oglas"]))) {
   $msg = "Oglas mora biti odstranjen iz oglasnika, da ga lahko spreminjate!";
   $klasa = "alert-danger";
}
...
if (isset($_POST["ponovno_objavi_oglas"])) {
   ...
   if (date('Y-m-d') > $datum_ponovne_objave) {
      ...
      $poizvedba = "UPDATE oglasi SET datum_objave = ? WHERE id = ? AND id_uporabnika = ?";

      $stmt = $conn->prepare($poizvedba);
      $stmt->bind_param("sii", $dat_pon_objave, $_GET["oglas"], $_SESSION["login_id"]);
      $stmt->execute();

      $stmt->close();

      $poizvedba = "UPDATE oglasi SET datum_poteka = ?  WHERE id = ? AND id_uporabnika = ?";

      $stmt = $conn->prepare($poizvedba);
      $stmt->bind_param("sii", $dat_pon_poteka, $_GET["oglas"], $_SESSION["login_id"]);
      $stmt->execute();
...
if (isset($_POST["odstrani_oglas"])) {
   ...
   if (/*$razlika > 0 ||*/ date('Y-m-d') < $datum_poteka_pomozna) {
      ...
      $stmt = $conn->prepare($poizvedba);
      $stmt->bind_param("sii", $datum, $_GET["oglas"], $_SESSION["login_id"]);
      $stmt->execute();

      $stmt->close();

      $msg = "Oglas je bil odstranjen iz oglasnika!";
      $klasa = "alert-success";
   }
   else {
...
if (isset($_POST["izbrisi_oglas"])) {
   ...
   $poizvedba = "DELETE FROM oglasi WHERE id = ? AND id_uporabnika = ?";
   ...
   $poizvedba = "UPDATE uporabniki SET st_vseh_objav = st_vseh_objav-1 WHERE id = ?";
   ...
   header("location: 6_ogledoglasov.php");
}
?>
...
<section class="section_3" style="padding-bottom: 5px;">
   <?php
      if (isset($_GET["oglas"])) {
         $poizvedba = "SELECT id, naziv, opcija, kategorija, opis, cena, stanje, datum_objave, datum_poteka FROM oglasi WHERE id = ? AND id_uporabnika = ? LIMIT 1";

         $stmt = $conn->prepare($poizvedba);
         $stmt->bind_param("ii", $_GET["oglas"], $_SESSION["login_id"]);
         $stmt->execute();
         $stmt->bind_result($id, $naziv, $opcija, $kategorija, $opis, $cena, $stanje, $datum_objave, $datum_poteka);
         $stmt->fetch();
      ...
      <form action="" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
         ...
         <div style="text-align: center; margin-top: 10px;">
            <?php if (isset($slika1) || isset($slika2) || isset($slika3) || isset($slika4) || isset($slika5)) { ?>
               <input type="submit" name="odstrani" onclick="return confirm('Ali res želite odstraniti te slike oglasa?');" value="ODSTRANI IZBRANE SLIKE" class="btn btn-primary btn-sm" style="font-weight: bold; padding: 7px;" />
            <?php } ?>
         </div>
         <div style="text-align: center; margin-top: 10px;">
            <?php if (!isset($slika1) || !isset($slika2) || !isset($slika3) || !isset($slika4) || !isset($slika5)) { ?>
   ...
   <?php if (!empty($msg)) { ?>
      <br/>
      <br/>
      <div style="border-radius: 10px;" class ="alert <?php echo $klasa; ?>">
         <?php echo $msg; ?>
      </div>
   <?php }
      }
